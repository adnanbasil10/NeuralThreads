...
export default function DesignerChatsPage() {
    ...
    const connectSocketWithUser = async () => {
      try {
        ...
        if (userData.success && userData.data) {
          ...
          connectSocket(user.id, user.name || 'Designer', user.role || 'DESIGNER');
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      try {
        const res = await secureFetch(`/api/chat/messages?chatId=${selectedChat.id}`, {
          credentials: 'include',
        });
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (currentUserIdRef.current) {
        sendStopTyping(selectedChat.id, currentUserIdRef.current, 'Designer');
      }
  ...
  const handleTyping = async () => {
    if (selectedChat && currentUserIdRef.current) {
      try {
        ...
        const userData = await userRes.json();
        if (userData.success && userData.data) {
          sendTyping(selectedChat.id, userData.data.id, userData.data.name || 'Designer');
        }
      } catch (error) {
